...
# Point reticulate to your preferred Python environment
# use_virtualenv("darts-env", required = FALSE)
# use_condaenv("darts-conda", required = FALSE)

# Ensure darts is available; uncomment to install into the active env
# py_install("u8darts", pip = TRUE)
```
...
sales_df <- readRDS(data_file) |>
  ...
  transmute(
    ds        = as.Date(date),
    unique_id = id,         # keep as-is or paste(item_id, store_id, sep = "_")
    y         = units
  ) |>
...
for ts in series_list:
    # Generate date-based features (month, day-of-week)
    ...
    cov = cov.with_columns_renamed({c: f"cal_{c}" for c in cov.columns})
    cov = cov.slice_intersect(ts)  # align
...
model.fit(
    ...
    future_covariates=covariates_list if use_covariates else None,
    verbose=True
)
```
